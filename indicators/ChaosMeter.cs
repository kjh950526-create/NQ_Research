...
namespace NinjaTrader.NinjaScript.Indicators
{
	public class ChaosMeter : Indicator
	{
		...
		private int lastWinTicks = 0;       // ★ 마지막 계산창 틱 개수 (완전성 표시용)

		// ★ 안전장치 상수(§6.124)
		private const int MinWindowTicks  = 550;  // 창 전체 최소 1초틱 (완전=600, 여유 550)
		private const int MinBarsPerMeter = 50;   // 미터당 최소 10초봉 (완전=60, 여유 50)
		...
		{
			// ★ 안전장치(§6.124): 창 전체 1초 틱 완전성 체크
			//   완전한 10분 = 600초. 로딩 지연/렉으로 틱이 드문드문 들어오면
			//   적은 데이터로 crossing이 낮게(초록) 나와 확정 고정되는 버그 방지.
			//   충분한 틱(기본 550개+)이 없으면 계산 거부 → NaN(데이터 부족 표시).
			DateTime ds0 = curDay + start;
			DateTime de0 = curDay + end;
			int winTicks = 0;
			foreach (Tick tk in buffer)
				if (tk.T > ds0 && tk.T <= de0) winTicks++;
			lastWinTicks = winTicks;   // 패널 표시용
			if (winTicks < MinWindowTicks) return double.NaN;   // ★ 불완전 데이터 → 계산 거부
		...
		private int ComputeOneMeter(TimeSpan start, TimeSpan end, int offsetSec)
		{
			...
			if (bars.Count < MinBarsPerMeter) return -1;   // ★ 강화(§6.124): 6→50, 불완전창 거부
		...
		protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
		{
			...
			// ★ 안전장치(§6.124): NaN이면 데이터 부족 경고 + 틱 개수 표시
			//   로딩 지연/렉으로 불완전 계산된 경우 명확히 알림 (초록 오판 방지)
			string line1;
			if (double.IsNaN(meterValue))
			{
				if (finalizedToday)
					line1 = string.Format("CHAOS: 데이터부족! ({0}틱<{1})", lastWinTicks, MinWindowTicks);
				else
					line1 = string.Format("CHAOS: {0}", statusText);
			}
			else
			{
